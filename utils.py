...
def loadData(dataset):
    ...
    for _ in range(10):  # No need to vary seed in loop
        x_train, x_test, y_train, y_test = train_test_split(
            x_class, y_class, test_size=0.95, random_state=42
        )
        ...
        classData = {
            'x_train': x_train,
            'x_test': x_test,
            'y_train': if dataset!='Salinas_corrected':y_train - 1 else : y_train,
            'y_test': if dataset!='Salinas_corrected':y_test - 1 else : y_test
        }
